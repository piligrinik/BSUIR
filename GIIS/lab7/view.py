...
from lab1.cube import Cube, CubeApp
from lab1.polygon import PolygonEditor
from lab1.DelaunayView import DelaunayApp
from lab1.VoronoiView import VoronoiApp
...
buttons = ["Draw the line", "Draw the circle", "Draw the ellipse", "Draw the hyperbola", "Draw the parabola",
           "Draw the curve", "Cube transfer", "Draw the polygon", "Delaunay triangulation",
           "Voronoi diagram", "smth", "smth"]
button_lines = ttk.Button(root, text=buttons[0], command=lambda: open_lines_window(), width=28)
button_lines.place(relx=0.1, rely=0.05, anchor="w")
button_circle = ttk.Button(root, text=buttons[1], command=lambda: open_circle_coords(), width=28)
button_circle.place(relx=0.1, rely=0.12, anchor="w")
button_ellipse = ttk.Button(root, text=buttons[2], command=lambda: open_ellipse_coords(), width=28)
button_ellipse.place(relx=0.1, rely=0.19, anchor="w")
button_hyperbola = ttk.Button(root, text=buttons[3], command=lambda: open_hyperbola_coords(), width=28)
button_hyperbola.place(relx=0.1, rely=0.26, anchor="w")
button_parabola = ttk.Button(root, text=buttons[4], command=lambda: open_parabola_coords(), width=28)
button_parabola.place(relx=0.1, rely=0.33, anchor="w")
button_curve = ttk.Button(root, text=buttons[5], command=lambda: open_curves_window(), width=28)
button_curve.place(relx=0.1, rely=0.40, anchor="w")
button_cube = ttk.Button(root, text=buttons[6], command=lambda: cube_transfer(), width=28)
button_cube.place(relx=0.1, rely=0.47, anchor="w")
button_polygon = ttk.Button(root, text=buttons[7], command=lambda: draw_polygon(), width=28)
button_polygon.place(relx=0.1, rely=0.54, anchor="w")
button_triangulation = ttk.Button(root, text=buttons[8], command=lambda: triangulation(), width=28)
button_triangulation.place(relx=0.1, rely=0.61, anchor="w")
button_diagram = ttk.Button(root, text=buttons[9], command=lambda: diagram(), width=28)
button_diagram.place(relx=0.1, rely=0.68, anchor="w")

for i, btn_text in enumerate(buttons[10:]):
    btn = ttk.Button(root, text=btn_text, width=28)
    btn.place(relx=0.1, rely=0.75 + i * 0.07, anchor="w")
...
def open_lines_window(x0, y0, x1, y1, cell_size):
    ...
    button1 = ttk.Button(new_window, text="DDA method",
                         command=lambda: lineDrawer.draw_line(1, x0, y0, x1, y1, cell_size))
    button1.pack(pady=10)
    debug_button1 = ttk.Button(new_window, text="Debug of DDA",
                               command=lambda: lineDrawer.start_debug(1, x0, y0, x1, y1, cell_size))
    debug_button1.pack(pady=10)

    button2 = ttk.Button(new_window, text="Bresenham's method",
                         command=lambda: lineDrawer.draw_line(2, x0, y0, x1, y1, cell_size))
    ...
    button3 = ttk.Button(new_window, text="Wu's method",
                         command=lambda: lineDrawer.draw_line(3, x0, y0, x1, y1, cell_size))
    button3.pack(pady=10)
    debug_button3 = ttk.Button(new_window, text="Debug of Wu's",
                               command=lambda: lineDrawer.start_debug(3, x0, y0, x1, y1, cell_size))
...
def open_lines_coord_window():
    ...
    draw_button = ttk.Button(root2, text="Draw the line",
                             command=lambda: open_lines_window(int(entry_x0.get()), int(entry_y0.get()),
                                                               int(entry_x1.get()), int(entry_y1.get()),
                                                               int(entry_cell_size.get())))
...
def cube_transfer():
    ...
    root_cube = tk.Toplevel(root)
    root_cube.title("3D Кубик с правильными преобразованиями")
    app = CubeApp(root_cube, cube)
...
def draw_polygon():
    root2 = tk.Toplevel()
    app = PolygonEditor(root2)


def triangulation():
    root_child = tk.Toplevel()
    app = DelaunayApp(root_child)

def diagram():
    root_child = tk.Toplevel()
    app = VoronoiApp(root_child)
